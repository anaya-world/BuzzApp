...
import com.example.myapplication.Utils.RingtonePlayer;
...
ringtonePlayer=new RingtonePlayer(this);
        ...
        if (mIncomingCallId != null) {  // as callee
            ...
            mCalleName=getIntent().getStringExtra(ActivityUtils.EXTRA_CALLEE_NAME);
            ...
            String name = mDirectCall.getCaller().getNickname();
            String image = mDirectCall.getCaller().getProfileUrl();
            String userid = mDirectCall.getCaller().getUserId();
            ...
            databaseHelper.insertdata(randomInt, name, image, "incoming", sb.toString(), userid);
    ...
    protected void setViews() {
        mImageViewDecline.setOnClickListener(view -> {
            Log.d(TAG, "mImageViewDecline clicked =>end"+mDirectCall);
            if (mDirectCall != null) {
                end(mDirectCall);
            }
        });
        ...
        mImageViewAccept.setOnClickListener(view -> {
            ...
            Log.d(TAG, "mImageViewAccept clicked =>startCall");
        ...
        mImageViewEnd.setOnClickListener(view -> {
            Log.d(TAG, "mImageViewEnd clicked =>end");
    ...
    protected boolean setState(STATE state, DirectCall call) {
        ...
        switch (state) {
            case STATE_INCOMING: {
                Log.d(TAG, "setState() => STATE_INCOMING()");
            ...
            case STATE_OUTGOING: {
                Log.d(TAG, "setState() => STATE_OUTGOING()");
                mLinearLayoutInfo.setVisibility(View.VISIBLE);
            ...
            case STATE_ENDING: {
                Log.d(TAG, "setState() => STATE_ENDING()");
                if (mIsVideoCall) {
            ...
            case STATE_ENDED: {
                Log.d(TAG, "setState() => STATE_ENDED()");
                mLinearLayoutInfo.setVisibility(View.VISIBLE);
                mImageViewProfile.setVisibility(View.VISIBLE);
                mLinearLayoutRemoteMute.setVisibility(View.GONE);
                mRelativeLayoutRingingButtons.setVisibility(View.GONE);
    ...
    protected void setInfo(DirectCall call, String status) {
        ...
        if (remoteUser != null) {
            ...
            mTextViewUserId.setText(mCalleName);
        }
