...
public class PushUtils {
    ...
    public static void getPushToken(Context context, final GetPushTokenHandler handler) {
        ...
        if (TextUtils.isEmpty(savedToken)) {
            FirebaseApp calls = FirebaseApp.getInstance("calls");
            FirebaseInstanceId.getInstance(calls).getInstanceId().addOnCompleteListener(task -> {
    ...
    public static void registerPushToken(Context context, String pushToken, PushTokenHandler handler) {
        ...
        SendBirdCall.registerPushToken(pushToken, false, e -> {
            ...
            SendBird.registerPushTokenForCurrentUser(pushToken, new SendBird.RegisterPushTokenWithStatusHandler() {
                @Override
                public void onRegistered(SendBird.PushTokenRegistrationStatus pushTokenRegistrationStatus, com.sendbird.android.SendBirdException e) {
                    Log.d(TAG, "registerPushTokenForCurrentUser() ");
                    if (e != null) {
